...
public class TagJdbcRepository implements TagRepository {
    ...
    public List<TagEntity> createPostTags(@NonNull Long postId, @NonNull List<String> tagsToBeAdded) {
        ...
        if (!tagsToBeAdded.isEmpty()) {
            var tagsToBeAddedTextArray = DbUtil.convertToNativePostgreSqlTextArray(jdbcTemplate, tagsToBeAdded);
            var sql = """
                        WITH given_tags(t_name) AS (SELECT UNNEST(?::text[])),
                        inserted_tags AS (
                    ...
                    ORDER BY at.name;
                    """;
            addedTagEntities = jdbcTemplate.query(sql, new TagEntityRowMapper(), tagsToBeAddedTextArray, postId);
        }
    ...
    public List<TagEntity> findAllByPostId(@NonNull Long postId) {
        ...
        var sql = """
                ...
                WHERE pt.post_id = ?
                """;
        var tagEntities = jdbcTemplate.query(sql, new TagEntityRowMapper(), postId);
    ...
    public void unlinkAllTagsFromPost(@NonNull Long postId) {
        ...
        var sql = "DELETE FROM post_tag WHERE post_id = ?";
